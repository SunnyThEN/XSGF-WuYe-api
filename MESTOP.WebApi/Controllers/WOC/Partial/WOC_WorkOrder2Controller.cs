...
*如: [ApiActionPermission("WOC_WorkOrder2",Enums.ActionPermissionOptions.Search)]
 */
...
{
    public partial class WOC_WorkOrder2Controller
    {
        private readonly IWOC_WorkOrder2Service _service;//访问业务代码
        private readonly IHttpContextAccessor _httpContextAccessor;

        [ActivatorUtilitiesConstructor]
        public WOC_WorkOrder2Controller(
            IWOC_WorkOrder2Service service,
        ...
        public ActionResult Getmes_workordermainRootData([FromBody] PageDataOptions options)
        {
            ...
            var rows = query.TakeOrderByPage(options.Page, options.Rows)
                .OrderByDescending(x => x.CreateDate).Select(s => new
                {
                    ...
                    hasChildren = roleRepository.Where(x => x.ParentId == s.MWOId && x.Orderlevel == 4).ToList()
        ...
        public async Task<ActionResult> GetChildrenData(int MWOId)
        {
            ...
            var rows = await roleRepository.Where(x => x.ParentId == MWOId && x.Orderlevel == 2)
                .Select(s => new
                {
                    s.MWOId,
                    s.WorkOrderCode,
        ...
        public ActionResult WOsplit([FromBody] List<Search_PPM_WorkFlow> workflow, string ParentWorkOrderCode)
        {
            ...
            foreach (var wf in workflow)
            {
                try
                {
                    ...
                    wkorder.ProcessFlowName = wf.ProcessFlowName;
