...
*如: [ApiActionPermission("WOC_WorkOrderMain",Enums.ActionPermissionOptions.Search)]
 */
...
{
    public partial class WOC_WorkOrderMainController
    {
        private readonly IWOC_WorkOrderMainService _service;//访问业务代码
        private readonly IHttpContextAccessor _httpContextAccessor;

        [ActivatorUtilitiesConstructor]
        public WOC_WorkOrderMainController(
            IWOC_WorkOrderMainService service,
        ...
        {
            var roleRepository = WOC_WorkOrderMainRepository.Instance.FindAsIQueryable(x => 1 == 1);

            //页面加载根节点数据条件x => x.ParentId == 0,自己根据需要设置
            var query = WOC_WorkOrderMainRepository.Instance.FindAsIQueryable(x => x.ParentId == 0 || x.ParentId == null);
