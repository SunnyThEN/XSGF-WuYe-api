...
using MESTOP.Core.UserManager;
using System.Linq;

namespace MESTOP.Core.Infrastructure
{
    public static class DictionaryHandler
    {
        ...
        public static string GetCustomDBSql(string dicNo, string originalSql)
        {
            switch (dicNo)
            {
                ...
                case "tree_roles":
                    ...
                    break;
				 case "人员选择":
                    originalSql = GetUserSql(originalSql);
                    break;
                case "参与人选择":
                    originalSql = GetUserSql(originalSql);
                    break;
       ...
       public static string GetUserSql(string originalSql)
        {
            originalSql = "SELECT cast([User_Id]  as varchar(50)) as [key],UserTrueName as [value] FROM [Sys_User]  where 1=1 ";
            ...
            var roleIdList = roleIds.ConvertAll<string>(s => "'" + s.ToString() + "'");
            //string sqlstr= string.Empty;
     
            // for(int i=0;i< roleIds.Count; i++)
            //{

            //        if (i == 0) { sqlstr = " '" + roleIds[i] + "' in [RoleIds] " ; }
            //        else 
            //        {
            //            sqlstr = sqlstr+" or '" + roleIds[i] + "' in [RoleIds] "  ;
          //  //        }
          //  var rolestr=string.Empty;
          //foreach(var role in roleIdList)
          //  {

          //      rolestr = rolestr + " or CHARINDEX("+ role + ", [RoleIds] ) > 0";
          //  }

            var conditions = roleIdList.Select(role => $"CHARINDEX({role}, [RoleIds]) > 0");
            string rolestr = string.Join(" OR ", conditions);

            //}
            string sql = $@" {originalSql}  and ("+ rolestr + ")";

            //string sql = $@" {originalSql} "+ rolestr;
            return sql;
        }
